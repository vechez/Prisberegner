...
(function(){
  ...
  root.innerHTML = `
    <div class="card" role="region" aria-label="Arbejdsskade - prisberegner">
      ...
      <div class="body">
        ...
        <section class="pane" data-step="2" hidden>
          <div class="grid">
            <div class="row">
              <div><label for="antal">Antal medarbejdere</label>
        ...
        <section class="pane" data-step="3" hidden>
          <div class="two-col">
            <div class="grid">
              ...
              <div class="total"><div class="total-label">Årlig pris (inkl. gebyrer og afgifter)</div><div class="total-amount" id="total">0 kr.</div></div>
              <div id="price-disclaimer" class="disclaimer">Prisen er årlig og inkluderer alle gebyrer og afgifter. Den viste pris er vejledende og ikke garanteret, da skadeshistorik og øvrige forsikringsforhold kan påvirke den endelige pris. Priserne er baseret på tilbud fra en af vores mange samarbejdspartnere.</div>
    ...
    <div id="bridge" class="bridge-overlay" aria-hidden="true"><div class="bridge-box"><div class="bridge-title">Beregner pris…</div><div class="meter"><span></span></div><div class="bridge-hint">Et øjeblik – vi samler dine valg</div></div></div>
  `;
  ...
  async function fetchVirkByCVR(cvr){
    try{
      ...
      if(d && (d.cvr || d.vat)) return {
        ...
        branchekode: d.industrycode || d.main_industrycode || d.branchekode || null,
        ansatte: (typeof d.employees === 'number' ? d.employees : (d.employeesYear || d.antal_ansatte || null))
      };
    }catch(e){ return null; }
    return null;
  }
  ...
  function makeCombo(host, idx){
    ...
    const closeList=()=>{ list.style.display='none'; input.setAttribute('aria-expanded','false'); open=false; cursor=-1; };
    const render=(items)=>{ list.innerHTML=''; items.slice(0,300).forEach((o,i)=>{ const el=document.createElement('div'); el.className='combo-opt'; el.setAttribute('role','option'); el.textContent = o.label; el.onclick=()=>{ input.value=o.label; state.roles[idx]=o.label; closeList(); }; if(i===cursor) el.setAttribute('aria-selected','true'); list.appendChild(el); }); };
    const filter=(q)=>{ const s=(q||'').toLowerCase(); opts = POS.filter(o=> o.label.toLowerCase().includes(s)); render(opts); if(opts.length) openList(); else closeList(); };
    input.addEventListener('input', e=>{ filter(e.target.value); });
    input.addEventListener('focus', ()=>{ filter(input.value); });
    input.addEventListener('keydown', e=>{ if(!open && ['ArrowDown','Enter'].includes(e.key)){ filter(input.value); return; } if(e.key==='Escape'){ closeList(); return;} if(!open) return; if(e.key==='ArrowDown'){ cursor=Math.min(cursor+1, opts.length-1); render(opts);} else if(e.key==='ArrowUp'){ cursor=Math.max(cursor-1,0); render(opts);} else if(e.key==='Enter'){ if(opts[cursor]){ input.value=opts[cursor].label; state.roles[idx]=opts[cursor].label; closeList(); } } });
    document.addEventListener('click', e=>{ if(!host.contains(e.target)) closeList(); }, {passive:true});
  }

  function renderRoleSelectors(){
    const sel = $('#antal'); if(sel.children.length===0){ sel.innerHTML = Array.from({length:10},(_,i)=>`<option value="${i+1}">${i+1}</option>`).join('') + '<option value="10+">10+</option>'; }
    const container = document.getElementById('roles'); const prev = [...state.roles]; container.innerHTML='';
    const v = sel.value || '1'; state.antal = (v==='10+'?10:parseInt(v,10));
    state.roles = new Array(state.antal).fill('');
    for(let i=0;i<state.antal;i++){
      const wrap=document.createElement('div'); wrap.className='item';
      const left=document.createElement('div'); left.innerHTML='<strong>Medarbejder ' + (i+1) + '</strong>';
      const right=document.createElement('div'); makeCombo(right, i);
      wrap.appendChild(left); wrap.appendChild(right); container.appendChild(wrap);
      if(prev[i]){ const input = right.querySelector('input'); input.value = prev[i]; state.roles[i]=prev[i]; }
    }
    postHeight();
  }

  function calculateTotal(){
    let sum=0; const list=document.getElementById('breakdown'); list.classList.add('role-list'); list.innerHTML='';
    const byLabel = new Map(POS.map(o=>[o.label,o.price]));
    for(let i=0;i<state.roles.length;i++){
      const r=state.roles[i]; const p = byLabel.get(r) || 0; sum += p;
      const row=document.createElement('div'); row.className='role-card';
      row.innerHTML = '<div class="idx">'+(i+1)+'</div><div class="role-chip">'+(r||'—')+'</div><div class="price-pill">'+money(p)+'</div>';
  ...
  function init(){
    ...
    const handleCVRInput = debounce(async (val)=>{
      if(val.length===8){
        const box = document.getElementById('virk-box'); box.textContent = 'Henter virksomhedsdata…';
        const v = await fetchVirkByCVR(val);
        if(v){ state.virk=v; state.cvr=val; box.innerHTML =
        ...
        else { box.innerHTML='<div class="muted">Kunne ikke hente virksomhedsdata. Vi indhenter det manuelt efterfølgende.</div>'; }
        postHeight();
      }
    }, 450);
    ...
    function handleSubmit(){
      ...
      parent.postMessage({ type:'FF_CALC_EVENT', event:'lead_submitted', value: state.total }, '*');
      postHeight();
    }
